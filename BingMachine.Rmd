...
We eventually decided to include children who have completed 75% (i.e., 15 trials) of the test trials. 
...
bing_individual <- bingvalid %>%
  group_by(ID, Gender, Age.months, Midtesting.age.months, Midtesting_age_group) %>% 
...
```{r}
mean(bing_individual$Age.months)
sd(bing_individual$Age.months)
min(bing_individual$Age.months)
max(bing_individual$Age.months)
table(bing_individual$Age.months)
```
...
```{r}
mean(bing_individual$Midtesting.age.months)
sd(bing_individual$Midtesting.age.months)
min(bing_individual$Midtesting.age.months)
max(bing_individual$Midtesting.age.months)
table(bing_individual$Midtesting.age.months)
```
...
```{r}
median_split <- ifelse(bing_individual$Midtesting.age.months <= 49, "young", "old")
table(median_split)
```
...
# Warm-up
```{r}
levels(bingvalid$Training.Crit.reached)
```
